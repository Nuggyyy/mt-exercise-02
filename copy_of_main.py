...
train_metrics_file = os.path.join(args.metrics_dir, f'train_dropout_{args.dropout:.2f}.csv')
valid_metrics_file = os.path.join(args.metrics_dir, f'valid_dropout_{args.dropout:.2f}.csv')
test_metrics_file = os.path.join(args.metrics_dir, f'test_dropout_{args.dropout:.2f}.csv')
...
def train():
    ...
    final_loss = total_loss / args.log_interval
    return final_loss
...
def initialize_metrics_files():
    ...
    # Valid metrics file
    with open(valid_metrics_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Epoch', 'Perplexity'])

def log_train_metrics(epoch, train_ppl):
    """Log training metrics to file"""
    with open(train_metrics_file, 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([epoch, f"{train_ppl:.6f}"])

def log_valid_metrics(epoch, valid_ppl):
    """Log validation metrics to file"""
    with open(valid_metrics_file, 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([epoch, f"{valid_ppl:.6f}"])

def log_test_metrics(test_ppl):
    """Log test metrics to file"""
    with open(test_metrics_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Model', 'Perplexity'])
        writer.writerow([f'dropout_{args.dropout:.2f}', f"{test_ppl:.6f}"])

# Before the training loop, initialize metrics files
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        log_train_metrics(epoch, train_ppl)
        log_valid_metrics(epoch, val_ppl)
